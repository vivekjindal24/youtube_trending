...
\title{\textbf{Can We Predict Which YouTube Videos Go Viral? A Machine Learning Investigation}}
...
\begin{abstract}
Here's what we wanted to know: can you look at a YouTube video's metadata—its title, engagement numbers, upload time—and predict whether it'll hit the trending page? We used a dataset of 36,719 videos and built a classification pipeline to find out. The short answer: yes, but with caveats. Our Gradient Boosting model achieved 94\% accuracy and an F1-score of 0.861, substantially beating simpler baselines. What stood out immediately was that engagement \textit{rates} (likes per view, not just raw like counts) mattered more than we expected. Text features from descriptions helped too, though not as much as the engagement signals. The bigger question—whether this tells us anything about \textit{why} videos go viral—is harder to answer. We'll discuss what worked, what didn't, and where this approach breaks down.
\end{abstract}
...
\section{Background: What We Know About Classification and Virality}
...
The trade-off: interpretability vs. performance. Logistic regression coefficients tell you directly which features matter. Tree ensembles give you better predictions but make it harder to explain \textit{why}.

\subsection{Evaluation Metrics Matter More Than You'd Think}
...
\subsection{Feature Engineering: The Underrated Step}
...
\begin{itemize}
\item \textbf{Metadata}: Video ID, language, country (mostly India in this dataset), publish date.
\item \textbf{Engagement}: View count, like count, comment count. These are the big signals.
\item \textbf{Text}: Title and description. Free-form text that required preprocessing.
\item \textbf{Ranking}: Daily rank within trending videos. This is how we defined our target.
\end{itemize}

The dataset isn't perfect. It's geographically concentrated (lots of India, less global diversity). We don't know \textit{when} the engagement metrics were measured—hours after upload? Days? That ambiguity matters for practical prediction but is unavoidable with this data.

\subsection{Defining "Trending"}

We created a binary target: \texttt{is\_trending = 1} if a video ranked 10th or better, otherwise 0. This threshold is somewhat arbitrary but balances competing needs: enough positive examples for training (about 15\% of the dataset), and focusing on truly high-performing content rather than marginal cases.
...
Figure~\ref{fig:class_dist} shows the resulting class distribution: 85\% non-trending, 15\% trending. Moderately imbalanced but manageable with stratified sampling and balanced class weights.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/figure_4_class_distribution.png}
    \caption{Class distribution in our dataset. Most videos don't trend (85\%), which mirrors real-world conditions. We used stratified splitting to maintain this balance in train/test sets.}
    \label{fig:class_dist}
\end{figure}

\subsection{Cleaning: Less Glamorous, More Necessary}
...
\section{How We Built Features}

\subsection{Text Features: More Than Just Word Counts}

Video titles and descriptions contain clues. We extracted:

\begin{itemize}
\item \textbf{Length stats}: Character counts, word counts for both title and description. Longer doesn't always mean better, but there's probably an optimal range.
\item \textbf{Punctuation indicators}: Binary flags for question marks and exclamation points in titles. These signal clickbait or engagement tactics that might correlate with virality.
\item \textbf{Viral keywords}: Presence of "official," "trailer," "live," "challenge" in titles. These terms pop up frequently in trending content.
\item \textbf{TF-IDF vectors}: For titles, top 3,000 terms. For descriptions, top 5,000. Unigrams and bigrams, excluding stopwords. This automated extraction lets the model discover patterns we didn't think to hand-code.
\end{itemize}
...
\subsection{Temporal Features: When Matters}
...
\subsection{Engagement Features: The Heavy Hitters}
...
\begin{itemize}
\item \texttt{likes\_per\_view} = like\_count / view\_count
\item \texttt{comments\_per\_view} = comment\_count / view\_count
\item \texttt{like\_to\_comment\_ratio} = like\_count / (comment\_count + 1)
\end{itemize}
...
\subsection{Categorical Features: Language Matters}

The \texttt{language} field got one-hot encoded. Different languages probably have different trending dynamics due to audience size, cultural preferences, or YouTube's recommendation algorithm. We let the model learn language-specific patterns rather than assuming they're all the same.
...
\begin{itemize}
\item \textbf{Numeric} (engagement counts, ratios, text stats): \texttt{StandardScaler} to zero mean, unit variance. Prevents features with larger scales from dominating.
\item \textbf{Categorical} (language): \texttt{OneHotEncoder} with unknown category handling.
\item \textbf{Text}: Separate \texttt{TfidfVectorizer} instances for title and description.
\end{itemize}
...
\subsection{Baseline: The Sanity Check}

We started with a \texttt{DummyClassifier} that always predicts the majority class (non-trending). This establishes the minimum acceptable performance. If a real model can't beat this, something is broken.
...
\subsection{Logistic Regression: The Linear Model}
...
The downside: it's linear. If trending depends on complex interactions (e.g., high engagement \textit{and} specific categories), logistic regression won't catch it unless we manually engineer those interaction terms.

\subsection{Random Forest: The Ensemble}
...
\subsection{Gradient Boosting: The Iterative Learner}
...
\subsection{Exploratory Analysis: Patterns in the Data}

Before modeling, we looked at distributions and relationships. Figure~\ref{fig:engagement_dist} shows engagement metrics are heavily right-skewed: most videos get modest engagement, a few get viral-level numbers. Log-scale transformations make the distributions look more normal, which suggests tree-based models (or log transformations) might work well.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/figure_1_engagement_distributions.png}
    \caption{Engagement distributions. Raw metrics are right-skewed (most videos cluster at low values). Log-scale reveals more structure. This skew motivated our use of engagement \textit{rates} rather than relying solely on raw counts.}
    \label{fig:engagement_dist}
\end{figure}

Figure~\ref{fig:engagement_by_trending} compares trending vs. non-trending videos on key metrics. Trending videos have significantly higher view counts (even on log scale), dramatically elevated likes-per-view, and higher comments-per-view. This visual analysis validates our hypothesis: both absolute engagement \textit{and} per-viewer engagement rates matter.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/figure_2_engagement_by_trending.png}
    \caption{Engagement metrics by trending status. Trending videos (class 1) dominate on all three metrics: log(views), likes-per-view, comments-per-view. The separation is stark, which bodes well for predictive models.}
    \label{fig:engagement_by_trending}
\end{figure}

Figure~\ref{fig:correlation} shows the correlation matrix. Strong correlations among raw engagement counts (view, like, comment)—multicollinearity is present. Engagement ratios show different patterns, justifying their inclusion as complementary features. Text length features have weak correlations with trending, suggesting they're not primary drivers (though they might still help at the margins).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/figure_3_correlation_heatmap.png}
    \caption{Correlation heatmap. Raw engagement metrics are highly correlated (multicollinearity). Engagement ratios less so, capturing different information. The target (is\_trending) correlates moderately with both absolute and rate-based features.}
    \label{fig:correlation}
\end{figure}

\subsection{Model Performance: The Numbers}

Table~\ref{tab:results} summarizes test set performance. All four models were trained on 29,375 videos and evaluated on 7,344 held-out videos.

\begin{table}[H]
\centering
\caption{Model performance on held-out test set. Gradient Boosting wins across all metrics.}
\label{tab:results}
...
Gradient Boosting & \textbf{0.942} & \textbf{0.875} & \textbf{0.848} & \textbf{0.861} & \textbf{0.938} \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:model_comparison} visualizes the performance gaps. The baseline's flat zero scores on precision/recall/F1 confirm it provides no discrimination. The progressive improvement from logistic → Random Forest → Gradient Boosting shows the value of increasing model sophistication.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/figure_6_model_comparison.png}
    \caption{Model comparison across four metrics. Gradient Boosting leads on everything. The baseline's zeros confirm it's a non-starter. The incremental gains from Random Forest to Gradient Boosting are modest but consistent.}
    \label{fig:model_comparison}
\end{figure}

\subsection{ROC Curves: Discrimination Ability}

Figure~\ref{fig:roc_curves} plots ROC curves for all models. All three trained models bow significantly above the diagonal (random chance). Gradient Boosting's curve lies furthest from the diagonal, confirming superior discrimination. The AUC values quantify this: 0.938 for Gradient Boosting, 0.922 for Random Forest, 0.907 for Logistic Regression, 0.500 for the baseline.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/figure_5_roc_curves.png}
    \caption{ROC curves. All trained models substantially outperform random chance (diagonal line). Gradient Boosting has the best AUC, though the differences are modest. The key takeaway: our features contain strong predictive signal.}
    \label{fig:roc_curves}
\end{figure}

\subsection{Error Patterns: Where Models Struggle}
...
\subsection{Prediction Function: Testing on New Videos}

We implemented a \texttt{predict\_trending\_status()} function that accepts video metadata and returns predictions + probabilities. Tested it on synthetic examples: a high-engagement music video with viral keywords got 85\%+ trending probability. A low-engagement tutorial got ~10\%. The model behaves sensibly.

\section{Discussion: What This Means (and Doesn't)}
...
Tree ensembles outperformed logistic regression, suggesting nonlinear feature interactions matter. A video might need \textit{both} high engagement \textit{and} a popular category to trend. Linear models can't capture that without explicit interaction terms.
...
\subsection{What Didn't Work (or Wasn't Tested)}
...
We didn't explicitly compute feature importance (could use SHAP values or permutation importance in future work). So while we can see \textit{that} engagement rates matter based on EDA and model comparisons, we can't quantify \textit{how much} each feature contributes to Gradient Boosting's predictions.

\subsection{Limitations We Can't Ignore}

\textbf{Causation vs. correlation:} High engagement causes trending, but trending also causes high engagement. It's a feedback loop. We can't disentangle causation with observational data alone. Controlled experiments (like A/B testing different titles) would help, but we don't have that here.

\textbf{Temporal ambiguity:} We don't know when engagement metrics were measured. If view counts and likes were recorded \textit{after} a video started trending, they're partially outcomes, not pure predictors. For a real-world prediction system, you'd need to specify the horizon: "Predict trending status 24 hours after upload based on metrics at hour 6."

\textbf{Geographic bias:} Dataset is heavily India-focused. Trending patterns vary by region due to language, culture, and local events. A model trained on this data might not generalize to other countries.

\textbf{Threshold sensitivity:} We defined trending as rank ≤ 10. Different thresholds (top 50, top 3) would yield different models. The choice of 10 is reasonable but arbitrary.

\textbf{Missing context:} No information about video content quality, thumbnails, external promotion, or creator influence. A popular channel with 10 million subscribers will trend more easily than a new channel, but we don't model that here.

\subsection{Practical Use Cases (With Caveats)}
...
\textbf{Hyperparameter tuning:} We used reasonable defaults but didn't systematically optimize. GridSearchCV or Bayesian optimization could squeeze out extra performance.

\textbf{Feature importance analysis:} SHAP or permutation importance would clarify which features drive predictions. Useful for understanding and for feature selection.

\textbf{Time-series modeling:} Instead of binary classification, model engagement trajectories over time. Predict \textit{when} a video will trend, not just \textit{if}.

\textbf{External data:} Incorporate social media mentions, search trends, news coverage. Virality often starts outside YouTube and spills over.

\textbf{Deep learning:} BERT or other transformers for text, CNNs for thumbnail images. Might capture semantic richness that TF-IDF misses. Trade-off: more data and compute required.

...
\title{\textbf{Predicting Viral Success on YouTube: A Machine Learning Approach to Trending Video Classification}}
...
\textbf{Identification and Metadata}: Each video has a unique identifier along with categorical attributes including language and country of origin (predominantly India in this dataset). Videos are also labeled with their publishing date, enabling temporal feature extraction.

\textbf{Engagement Metrics}: Quantitative signals capture audience response, including view count (total number of views received), like count (positive engagement actions), and comment count (discussion volume). These raw metrics form the foundation for derived engagement rate features.

\textbf{Textual Content}: Each video record includes the title (typical length 50-80 characters) and description (often several hundred characters) as free-form text fields. These provide semantic information about video content and creator communication style.

\textbf{Ranking Information}: The critical field for our prediction task is \texttt{daily\_rank}, which records the video's position in the trending chart. This enables definition of our binary target variable.

\subsection{Target Variable Definition}

We formulate trending prediction as binary classification by defining the target variable \texttt{is\_trending} through a threshold on \texttt{daily\_rank}. Specifically, a video is labeled as trending (class 1) if its rank is 10 or better, and non-trending (class 0) otherwise. This threshold balances several considerations: it focuses prediction on the most prominently featured content, creates a moderately imbalanced but not extreme class distribution (approximately 15\% trending, 85\% non-trending), and reflects a meaningful distinction between highly viral content and typical uploads.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/figure_4_class_distribution.png}
    \caption{Class distribution in the dataset. Left panel shows absolute counts; right panel shows percentage distribution. The moderately imbalanced distribution (85\% non-trending, 15\% trending) reflects real-world conditions where only a fraction of uploaded content achieves viral status.}
...
\textbf{Length statistics}: We compute character length and word count for both title and description. These features capture verbosity and may indicate professional production quality or adherence to platform best practices.

\textbf{Punctuation indicators}: Binary flags detect the presence of question marks and exclamation points in titles. Such punctuation often appears in attention-grabbing or clickbait-style titles that may correlate with viral potential.

\textbf{Viral keyword detection}: We construct binary features indicating whether specific keywords (\textit{official}, \textit{trailer}, \textit{live}, \textit{challenge}) appear in video titles. These terms frequently appear in trending content, capturing content categories known to attract large audiences.

\textbf{TF-IDF vectorization}: Beyond hand-crafted features, we apply TF-IDF transformation to convert titles and descriptions into high-dimensional numeric vectors. For titles, we extract the top 3,000 most informative terms; for descriptions, we use 5,000 terms. We employ unigrams and bigrams, capturing both individual words and two-word phrases, while excluding common English stopwords. This automated feature extraction allows the model to discover predictive linguistic patterns without requiring explicit enumeration.

\subsubsection{Temporal Features}

Upload timing may influence trending probability due to audience availability patterns. We extract the day of week from publication dates and create a binary \texttt{is\_weekend} indicator. These features enable the model to learn whether certain days exhibit higher trending rates, potentially due to leisure time availability or platform traffic patterns.

\subsubsection{Engagement Features}
...
\begin{itemize}
\item \texttt{likes\_per\_view} = \(\frac{\text{like\_count}}{\text{view\_count}}\): Measures what fraction of viewers actively liked the video
\item \texttt{comments\_per\_view} = \(\frac{\text{comment\_count}}{\text{view\_count}}\): Captures discussion intensity relative to viewership
\item \texttt{like\_to\_comment\_ratio} = \(\frac{\text{like\_count}}{\text{comment\_count} + 1}\): Compares passive appreciation to active discussion (adding 1 prevents division by zero)
\end{itemize}
...
The \texttt{language} field undergoes one-hot encoding, creating binary indicator variables for each observed language in the training data. This allows the model to learn language-specific trending patterns without imposing ordinality on what is inherently a nominal variable.

\subsection{Preprocessing Pipeline Architecture}

Our preprocessing pipeline handles the heterogeneous feature types through specialized transformers combined via \texttt{ColumnTransformer}:

\textbf{Numeric features} (engagement counts, ratios, length statistics, binary flags): Standardized via \texttt{StandardScaler} to zero mean and unit variance. This ensures features with different natural scales contribute comparably to distance-based algorithms and prevents features with larger numeric ranges from dominating the learning process.

\textbf{Categorical features} (language): Encoded via \texttt{OneHotEncoder} with unknown category handling enabled. This gracefully manages any languages in test data not observed during training by ignoring them rather than failing.

\textbf{Text features}: Processed through separate \texttt{TfidfVectorizer} instances for titles (3,000-dimensional output) and descriptions (5,000-dimensional output), yielding approximately 8,000+ total features after combining all transformations.
...
\subsubsection{Baseline: Dummy Classifier}

As a sanity check, we train a \texttt{DummyClassifier} that always predicts the majority class (non-trending). This trivial baseline establishes the minimum acceptable performance—any useful model must substantially exceed its accuracy.
...
\item \textbf{Precision}: Of predicted trending videos, what fraction actually trended
\item \textbf{Recall}: Of actual trending videos, what fraction were correctly identified
\item \textbf{F1-score}: Harmonic mean of precision and recall, providing a balanced metric that accounts for both false positives and false negatives
\item \textbf{ROC-AUC}: Area under the receiver operating characteristic curve, measuring the model's ability to discriminate between classes across all possible decision thresholds
\end{itemize}
...
Before modeling, we conducted exploratory analysis to understand feature distributions and relationships with the target variable. Figure~\ref{fig:engagement_dist} shows the distributions of key engagement metrics. All three metrics (views, likes, comments) exhibit highly right-skewed distributions, with most videos receiving modest engagement and a small fraction achieving viral-level metrics. Log-scale transformations reveal more approximately normal distributions, suggesting that tree-based models or log transformations may be beneficial.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{figures/figure_1_engagement_distributions.png}
    \caption{Distribution of engagement metrics across all videos. Top row: raw counts for views, views (log-scale), and likes. Bottom row: likes (log-scale), comments, and comments (log-scale). The heavy right skew in raw metrics motivates the use of normalized engagement rate features.}
    \label{fig:engagement_dist}
\end{figure}

Figure~\ref{fig:engagement_by_trending} compares engagement metrics between trending and non-trending videos. Trending videos exhibit significantly higher view counts (even on log scale), dramatically elevated likes-per-view ratios, and higher comments-per-view rates. This visual analysis confirms that both absolute engagement and per-viewer engagement rates serve as strong discriminative signals for trending status.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{figures/figure_2_engagement_by_trending.png}
    \caption{Comparison of engagement metrics between non-trending (class 0) and trending (class 1) videos. Trending videos show substantially higher log(view count), likes-per-view, and comments-per-view, validating these features as strong predictive signals.}
...
Table~\ref{tab:results} summarizes the quantitative performance of all four models on the 7,344-video test set. The results reveal clear performance stratification across modeling approaches.

\begin{table}[H]
\centering
\caption{Model Performance on Test Set (all metrics computed on held-out test data)}
\label{tab:results}
...
\subsubsection{Gradient Boosting Performance}
...
\subsection{Prediction on New Videos}
...
\subsection{Model Performance and Selection}
...
\subsection{Class Imbalance and Threshold Selection}
